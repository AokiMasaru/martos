 ...
 * File: task_manage.c
 * Category: MaRTOS
 * File Created: 2023/08/22 04:45
 * Author: Masaru Aoki ( masaru.aoki.1972@gmail.com )
 * *****
 * Last Modified: 2023/08/25 03:42
...
ID tk_cre_tsk( const T_CTSK *pk_ctsk )
{
    ...
    for(i = 0; i < CNF_MAX_TSKID; i++) {    // 未使用のTCBを検索
        if(tcb_tbl[i].status == TS_NONEXIST) break; 
    }
    /* TCBの初期化 */
    if(i < CNF_MAX_TSKID) {
        tcb_tbl[i].status    = TS_DORMANT;
        tcb_tbl[i].pre      = NULL;
        tcb_tbl[i].next     = NULL;
...
ER tk_sta_tsk( ID tskid, INT stacd )
{
    ...
    tcb = &tcb_tbl[tskid-1];
    if(tcb->status == TS_DORMANT) {  // タスクを実行できる状態に変更
        tcb->status = TS_READY;
...
void tk_ext_tsk( void )
{
    ...
    cur_task->status = TS_DORMANT;    // タスクを休止状態へ
