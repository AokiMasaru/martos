 ...
 * *****
 * Last Modified: 2023/09/12 04:44
 ...
 * Date      	By        	Comments
 * ----------	----------	----------------------------------------
 * 2023/09/12	Masaru Aoki	slp_tsk / wup_tsk実装
...
ER tk_wup_tsk( ID tskid )
{
    ...
    DI(intsts);     // 割込みの禁止
    tcb = &tcb_tbl[tskid-1];
    if((tcb->status == TS_WAIT) && (tcb->waifct == TWFCT_SLP)) { // tk_slp_tskで待ち状態か？
        ...
        scheduler();                                            // スケジューラの実行
    } else if(tcb->status == TS_READY || tcb->status == TS_WAIT) {      // 実行できる状態の場合
